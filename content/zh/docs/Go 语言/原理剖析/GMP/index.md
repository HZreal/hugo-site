...
![](static/ViGDb1hmso95TrxwDKycviLdnbb.png)

## GMP 模型

**线程是运行 goroutine 的实体，调度器的功能是把可运行的 goroutine 分配到工作线程上**。

![](static/NXdCbBGoeoOEyuxSFaVcdqh2ntb.png)
...
- runtime/debug 中的 SetMaxThreads 函数，设置 M 的最大数量，通常数量上 M >= P
- 一个 M 阻塞了，会创建新的 M。
...
优先将创建的 G 放入当前绑定的 P 的本地列表中

### G 发生阻塞调用时

M 执行的 G1 发生阻塞时（即 M 也是阻塞等待状态），当前 P 会和 M 解绑，从而可被其他的空闲 M1 进行绑定，空闲的 M1 被唤醒绑定该 P 后，从 G0 切换到该 P 的其他 G2 执行

![](static/DESJbfCopoan6dxc8abcIXacnmf.png)
